...
__device__ size_t ComputeIndex(size_t gid, CudaVec shape, CudaVec strides, size_t offset) {
    /*
    Compute the real **gid th** index in memory from the given gid and shape and strides
    */
    int dim_list[MAX_VEC_SIZE] = {0};

    for (int i = shape.size - 1; i >= 0; i--) {
        dim_list[i] = gid % shape.data[i];
        gid = gid / shape.data[i];
    }

    size_t _index = offset;
    for (int i = 0; i < shape.size; i++) {
        _index += dim_list[i] * strides.data[i];
    }
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  /// BEGIN SOLUTION
  size_t a_index = ComputeIndex(gid, shape, strides, offset);
  out[gid] = a[a_index];
  /// END SOLUTION
}
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t a_size, CudaVec shape,
                                   CudaVec strides, size_t offset) {
  /**
  ...
  size_t out_index = ComputeIndex(gid, shape, strides, offset);
  out[out_index] = a[gid];
  /// END SOLUTION
}
...
__global__ void ScalarSetitemKernel(size_t size, scalar_t val, scalar_t* out, CudaVec shape,
  ...
  *    size: number of elements to be copied
   ...
   *   shape: shapes of each dimension for out
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  /// BEGIN SOLUTION
  MatMulKernel<<<dim3(M, 1, 1), dim3(P, 1, 1)>>>(a.ptr, b.ptr, out->ptr, M, N, P);
  /// END SOLUTION
}
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t reduce_size) {
  ...
  for (int i = 1;i < reduce_size; i ++){
    if (a[gid * reduce_size + i] > out[gid]){
...
void ReduceMax(const CudaArray& a, CudaArray* out, size_t reduce_size) {
   ...
   */
  /// BEGIN SOLUTION
  ReduceMaxKernel<<<dim3(out->size, 1, 1), dim3(1, 1, 1)>>>(a.ptr, out->ptr, reduce_size);
  /// END SOLUTION
}
...
void ReduceSum(const CudaArray& a, CudaArray* out, size_t reduce_size) {
   ...
   */
  /// BEGIN SOLUTION
  ReduceSumKernel<<<dim3(out->size, 1, 1), dim3(1, 1, 1)>>>(a.ptr, out->ptr, reduce_size);
  /// END SOLUTION
}
